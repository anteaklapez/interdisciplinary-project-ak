...
def plot_histograms(df_results: pd.DataFrame):
...
def plot_log_histogram(df_results: pd.DataFrame):
    fig, ax = plt.subplots(figsize=(8,5))
...
def plot_ag_nag_ecdf(df_results: pd.DataFrame):
...
if __name__ == '__main__':
    ...
    plot_boxplot(df_results)
    plot_histograms(df_results)
    plot_log_histogram(df_results)
    plot_ag_nag_ecdf(df_results)
